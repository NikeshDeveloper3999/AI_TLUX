...
export const askAssistant = async (req, res) => {
    try {
        ...
        if (!user.geminiApiKey) {return res.status(400).json({ message: "gemini apikey is not added" })}
        ...
        if (user.enableNavigation) {
            ...
            if (wantsNavigation) {
                // Find matching page
                const matchedPage =user.pages.find((page) =>page.keywords.some((keyword) =cleanMessage.includes(keyword.toLowerCase())));
                // Page found
                if (matchedPage) {
                    // Already open
                    ...
                    // Navigate
                    return res.json({success: true,response:`Opening ${matchedPage.name}`});
                }
            }
        }


const prompt = `You are ${user.assistantName}.Business Name:${user.businessName}Business Type:${user.businessType}Business Description:${user.businessDescription}Assistant Tone:${user.tone}
Rules:- Keep replies under 15 words
...
const aiRespons = await generateGeminiResponse({ prompt, apikey: user.geminiApiKey, user })
